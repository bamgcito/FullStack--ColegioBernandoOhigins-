...
@Component({
  selector: 'app-alumno-horarios',
  standalone: true,
  imports: [CommonModule, IonContent, IonIcon, LayoutComponent],
  templateUrl: './alumno-horarios.page.html',
  styleUrls: ['./alumno-horarios.page.scss']
})
export class AlumnoHorariosPage implements OnInit {
  horarios: any[] = [];
  loading = true;
  ...
  ngOnInit() {
    ...
    this.api.getCursoDeAlumno(uid).subscribe({
      next: (curso: any) => {
